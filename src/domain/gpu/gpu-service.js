...
async function handleGpuCommand(runtime, normalized) {
  if (!isBareCommand(normalized.text, "/gpu")) {
...
async function handleSqCommand(runtime, normalized) {
  if (!isBareCommand(normalized.text, "/sq")) {
    await runtime.sendInfoCardMessage({
      chatId: normalized.chatId,
      replyToMessageId: normalized.messageId,
      text: "用法: `/sq`",
  ...
  try {
    ...
    const jobs = await slurmClient.listUserJobs(userName);
    await runtime.sendInfoCardMessage({
      chatId: normalized.chatId,
      replyToMessageId: normalized.messageId,
      text: buildSqSummaryText(userName, jobs),
    });
  } catch (error) {
    await runtime.sendInfoCardMessage({
      chatId: normalized.chatId,
...
function buildSqSummaryText(userName, jobs) {
  const header = ["JOBID", "ST", "NAME", "TIME", "NODELIST"];
  const rows = Array.isArray(jobs) ? jobs.map((job) => ([
    normalizeJobId(job.jobId) || "-",
    truncateText(job.state || "-", 2),
    truncateText(job.name || "-", 18),
    truncateText(job.time || "-", 10),
    truncateText(job.nodeList || "-", 20),
  ])) : [];

  const width = computeColumnWidths(header, rows);
  const lines = [formatRow(header, width)];
  if (!rows.length) {
    lines.push(formatRow(["-", "-", "暂无作业", "-", "-"], width));
  } else {
...
function isBareCommand(text, bareCommand) {
  const normalizedText = String(text || "").trim().toLowerCase();
  const normalizedCommand = String(bareCommand || "").trim().toLowerCase();
  return normalizedText === normalizedCommand;
}
